...
public class GridF : IGrid<float>
{
    ...
    internal void DrawData(Span<Point2d> corners, AABB bounds, float value, bool useApproximate)
    {
        if (useApproximate)
        {
    ...
    private void DrawDataFastApproximate(Span<Point2d> corners, AABB bounds, float value)
    {
        // 1. Convert Corners to Grid Space (Pixels)
        Span<Point2d> gridCorners = stackalloc Point2d[4];
        for (int i = 0; i < 4; i++)
        {
            var gx = (corners[i].X - Bounds.X) / XRes;
            var gy = (corners[i].Y - Bounds.Y) / YRes;
            gridCorners[i] = new Point2d(gx, gy);
        }
    ...
    private void DrawDataFastExact(Span<Point2d> corners, AABB bounds, float value)
    {
        // 1. Convert Corners to Grid Space (Pixels)
        Span<Point2d> gridCorners = stackalloc Point2d[4];
